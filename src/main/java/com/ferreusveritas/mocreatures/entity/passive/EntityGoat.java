...
public class EntityGoat extends MoCEntityTameableAnimal {
	...
	public void selectType() {
		...
		boolean snowy = BiomeDictionary.hasType(this.world.getBiome(new BlockPos(posX, posY, posZ)), Type.SNOWY);
		...
		if (!getUpset() && !getCharging()) {
			...
			if (entityplayer1 != null) {// Behaviour that happens only close to player :)
				...
				if (getType() > 4 && this.rand.nextInt(200) == 0) {
					EntityGoat entitytarget = (EntityGoat) getClosestEntityLiving(this, 14D);
