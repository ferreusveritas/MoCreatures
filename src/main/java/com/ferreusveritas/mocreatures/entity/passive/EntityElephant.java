...
public class EntityElephant extends MoCEntityTameableAnimal {
	...
	private byte temper;
	private static final DataParameter<Integer> TUSK_TYPE = EntityDataManager.<Integer>createKey(EntityElephant.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> STORAGE_TYPE = EntityDataManager.<Integer>createKey(EntityElephant.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> HARNESS_TYPE = EntityDataManager.<Integer>createKey(EntityElephant.class, DataSerializers.VARINT);
	...
	public boolean processInteract(EntityPlayer player, EnumHand hand) {
		...
		if (!stack.isEmpty() &&!getIsTamed() && !getIsAdult() && stack.getItem() == Items.SUGAR) {
			stack.shrink(1);
			if (stack.isEmpty()) {
				player.setHeldItem(hand, ItemStack.EMPTY);
			}
	...
	public boolean isMyHealFood(ItemStack stack) {
		return !stack.isEmpty() && stack.getItem() == Item.getItemFromBlock(Blocks.HAY_BLOCK);
	}
