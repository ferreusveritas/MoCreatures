...
public class EntityMole extends MoCEntityTameableAnimal {
	
	private static final DataParameter<Integer> MOLE_STATE = EntityDataManager.<Integer>createKey(EntityMole.class, DataSerializers.VARINT);
	
	public EntityMole(World world) {
		super(world);
		setSize(1F, 0.5F);
	}
	...
	protected void initEntityAI() {
		...
		tasks.addTask(2, new EntityAIWanderMoC2(this, 1.0D));
	...
	protected void entityInit() {
		super.entityInit();
		dataManager.register(MOLE_STATE, Integer.valueOf(0)); // state - 0 outside / 1 digging / 2 underground / 3 pick-a-boo
	...
	public boolean isOnDirt() {
		Block block =
				world.getBlockState(
						new BlockPos(MathHelper.floor(posX), MathHelper.floor(getEntityBoundingBox().minY - 0.5D), MathHelper
								.floor(posZ))).getBlock();
	...
	private void digForward() {
		...
		double newPosX =
				coordX + Math.cos((MoCTools.realAngle(rotationYaw - 90F) / 57.29578F)) * (Math.sin((rotationPitch - 90F) / 57.29578F) * x);
		double newPosZ =
				coordZ + Math.sin((MoCTools.realAngle(rotationYaw - 90F) / 57.29578F)) * (Math.sin((rotationPitch - 90F) / 57.29578F) * x);
		Block block =
				world.getBlockState(
	 ...
	 */
	public int getState() {
		return ((Integer)dataManager.get(MOLE_STATE)).intValue();
	}
	...
	public void setState(int i) {
		dataManager.set(MOLE_STATE, Integer.valueOf(i));
	}
