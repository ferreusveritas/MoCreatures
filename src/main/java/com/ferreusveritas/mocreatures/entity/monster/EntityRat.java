...
public class EntityRat extends MoCEntityMob {
    ...
    public boolean attackEntityFrom(DamageSource damagesource, float i) {
        ...
        if (entity != null && entity instanceof EntityLivingBase) {
            ...
            if (!this.world.isRemote) {
                List<EntityRat> list =
                        this.world.getEntitiesWithinAABB(EntityRat.class,
                                new AxisAlignedBB(this.posX, this.posY, this.posZ, this.posX + 1.0D, this.posY + 1.0D, this.posZ + 1.0D)
                                        .expand(16D, 4D, 16D));
                Iterator<EntityRat> iterator = list.iterator();
                do {
                    if (!iterator.hasNext()) {
                        break;
                    }
                    EntityRat entityrat = (EntityRat) iterator.next();
